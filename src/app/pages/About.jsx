...
import { Heart, Brain, Globe, Star, Award, Users, ArrowRight, CheckCircle2 } from "lucide-react";
      ...
      <section className="relative py-20 lg:py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            >
              <Badge className="bg-teal-100 text-teal-800 mb-6">{t('about')}</Badge>
              <h1 className="text-4xl lg:text-5xl font-bold mb-6">
                <span className="bg-gradient-to-r from-teal-600 to-emerald-600 bg-clip-text text-transparent">
                  {t('about_hero_title')}
                </span>
                <br />
                <span className="text-slate-800">{t('about_hero_title_2')}</span>
              </h1>
            ...
            >
              <div className="relative">
                ...
                <div className="absolute -bottom-4 -right-4 bg-white p-4 rounded-xl shadow-lg">
                  <p className="text-sm font-semibold text-slate-800">Founded 2023</p>
          ...
          <div className="grid lg:grid-cols-3 gap-12">
            ...
            >
              <div className="prose prose-lg max-w-none">
                <div className="bg-teal-50 p-8 rounded-2xl border-l-4 border-teal-500 mb-8">
                  ...
                  <p className="text-right mt-4 font-semibold text-teal-700">— Vanita Pande</p>
                </div>

                <h3 className="text-2xl font-bold mb-4 text-slate-800">{t('why_multilingual_matters')}</h3>
            ...
            >
              <Card className="mb-8 bg-white/80 backdrop-blur-sm border-0 shadow-lg">
                <CardContent className="p-6">
                  <div className="text-center mb-6">
                    ...
                    <h3 className="text-xl font-bold text-slate-800 mb-2">Vanita Pande</h3>
                    <p className="text-slate-600">{t('founder_title')}</p>
                  </div>
                  
                  <h4 className="font-semibold text-slate-800 mb-3">{t('certifications_training')}</h4>
                  <ul className="space-y-2 mb-6">
                    {credentials.map((credential) => (
                      <li key={credential} className="flex items-start">
                        <CheckCircle2 className="h-4 w-4 text-emerald-500 mt-1 mr-2 shrink-0" />
                        <span className="text-sm text-slate-700">{credential}</span>
                      </li>
                    ))}
                  </ul>

                  <div className="bg-gradient-to-r from-teal-500 to-emerald-600 p-4 rounded-lg text-white text-center">
                    <p className="font-semibold">500+ {t('lives_transformed')}</p>
                    <p className="text-sm text-teal-100">{t('across_countries')}</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-gradient-to-br from-purple-50 to-violet-50 border-0">
                <CardContent className="p-6">
                  <h4 className="font-bold text-slate-800 mb-4">{t('languages_we_heal_in')}</h4>
                  <div className="grid grid-cols-2 gap-2">
                    {["English", "हिंदी", "العربية", "Tagalog", "中文", "Indian English"].map((lang) => (
                      <Badge key={lang} className="justify-center bg-white/80 text-slate-700">
                        {lang}
                      </Badge>
                    ))}
                  </div>
                  ...
                  <div className={`lg:w-1/2 ${index % 2 === 0 ? 'lg:pr-12' : 'lg:pl-12'}`}>
                    <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
                      <CardContent className="p-6">
                        ...
                        <h3 className="text-xl font-bold mb-3 text-slate-800">{milestone.title}</h3>
