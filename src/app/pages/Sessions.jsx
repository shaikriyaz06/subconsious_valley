...
export default function Sessions() {
  ...
  const loadData = async () => {
    try {
      ...
      try {
        ...
        const userPurchases = await Purchase.filter({ user_email: userData.email, payment_status: 'completed' });
  ...
  const handleSessionAction = (session) => {
    ...
    if (hasAccess(session)) {
      ...
      window.location.href = createPageUrl("SessionPlayer") + `?session=${session.id}`;
    } else {
      // User needs to purchase
      window.location.href = createPageUrl("checkout") + `?session=${session.id}`;
    }
  };

  const filteredSessions = sessions.filter(session => {
    const categoryMatch = selectedCategory === "all" || session.category === selectedCategory;
    const languageMatch = selectedLanguage === "all" || session.languages?.includes(selectedLanguage);
                ...
                <Card className="h-full hover:shadow-2xl transition-all duration-300 border-0 bg-white/80 backdrop-blur-sm group">
                  ...
                  <CardContent className="p-6">
                    <p className="text-slate-600 mb-4 leading-relaxed h-20 overflow-hidden">
                      {getTranslated(session, 'description') || "Transform your mindset and unlock your potential with this powerful hypnotherapy session."}
                    </p>
                    ...
                    {/* Available Languages */}
                    <div className="mb-6">
                      <p className="text-sm font-medium text-slate-700 mb-2">{t('available_in')}</p>
                    ...
                    >
                      ...
                      {hasAccess(session) ? t('start_session') : (session.price > 0 ? `${t('purchase')} - ${formatPrice(session.price)}` : t('start_session'))}
                    </Button>
                  </CardContent>
                </Card>
              </motion.div>
